...
(function (factory) {
    ...
    console.log(m.startOf('jDay'));
    console.log(m.startOf('jWeek'));
    console.log(m.startOf('jYear'));
    ...
    function oldMomentFormat(mom, formatStr) {
        ...
        formatStr = formatStr.replace("M", "jM");
   
        formatStr = formatStr.replace("D", "jD");
   
        formatStr = formatStr.replace("Y", "jY");
        return moment(mom).format(formatStr); // oldMomentProto.format.call(mom, formatStr); // oldMomentProto defined in moment-ext.js
    }
...
var ss= formatDateWithChunk(date, chunks[i]);
            s += ss;
        }
    ...
    function formatDateWithChunk(date, chunk) {
        ...
        if (typeof chunk === 'string') { // a literal string
            ...
            if (chunk.token == 'j')return '';
            if (tokenOverrides[token]) {
                var tk = tokenOverrides[token](date); // use our custom token
                return tk;
            }
            var vl = oldMomentFormat(date, token);
            return vl;
        }
    ...
    function formatRangeWithChunks(date1, date2, chunks, separator, isRTL) {
        ...
        //middleStr1 = moment(date1).format('jYYYY/jM/jD');
        //middleStr2 = moment(date2).format('jYYYY/jM/jD');
    ...
    */
    var DayTableMixin = FC.DayTableMixin = {
        ...
        getCellDate: function (row, col) {
          var getcdi=  this.getCellDayIndex(row, col);
        var dayD=   this.dayDates[getcdi].clone();
        ...
        getCellDayIndex: function (row, col) {
            var index=row * this.daysPerRow + this.getColDayIndex(col);
            return index;
        },
...
var cme=this.colCnt - 1 - col;
        ...
        getDateDayIndex: function (date) {
            ...
            if (dayOffset < 0) {
             ...
             var dayInd=   dayIndices[0] - 1;
                return dayInd;
            }
            else if (dayOffset >= dayIndices.length) {
                var dayInd = dayIndices[dayIndices.length - 1] + 1;
                 return dayInd;
            }
            else {
                var dayInd = dayIndices[dayOffset];
                return dayInd;
            }
        },
        ...
        renderHeadDateCellHtml: function (date, colspan, otherAttrs) {
            var view = this.view;
            //alert(moment(date).format('jYYYY/jM/jD'));
            
           var st=  '' +
                '<th class="fc-day-header ' + view.widgetHeaderClass + ' fc-' + dayIDs[date.day()] + '"' +
                    (this.rowCnt == 1 ?
                        ' data-date="' +oldMomentFormat(date,'YYYY-MM-DD') + '"' ://' data-date="' + date.format('YYYY-MM-DD') + '"' :
                        '') +
                    (colspan > 1 ?
                        ' colspan="' + colspan + '"' :
                        '') +
                    (otherAttrs ?
                        ...
                        '') +
                '>' +
               htmlEscape(oldMomentFormat(date, this.colHeadFormat)) +// htmlEscape(moment(date).format(this.colHeadFormat)) +
                '</th>';
            return st;
        },
        ...
        renderBgCellHtml: function (date, otherAttrs) {
            ...
            return '<td class="' + classes.join(' ') + '"' +
                ' data-date="' +oldMomentFormat( date,'YYYY-MM-DD') + '"' + // if date has a time, won't format it
                (otherAttrs ?
                    ' ' + otherAttrs :
                    '') +
                '></td>';
    ...
    var View = FC.View = Class.extend({
        ...
        // Computes what the title at the top of the calendar should be for this view moment().format('jYYYY/jM/jD')
        computeTitle: function () {
            var start = this.calendar.applyTimezone(this.intervalStart);
            var end = this.calendar.applyTimezone(this.intervalEnd);
            return this.formatRange(
                {
                    // in case intervalStart/End has a time, make sure timezone is correct
                    start: start,//this.calendar.applyTimezone(this.intervalStart),
                    end: end //this.calendar.applyTimezone(this.intervalEnd)
                },
